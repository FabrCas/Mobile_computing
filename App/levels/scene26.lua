...
function scene:show( event )
	...
	myLevel:loadLevel("Level26") -- set your scene/level name here
	...
	for i=1,35 do
	obj[i] = {}
	obj[i] = myLevel:getLayerObject("Layer 1","B03_"..string.format(i))
	print("nMattoni creazione "..nMattoni)
	print("i mattoni = "..i)
...
elseif angolo <-45 and angolo >-135 then --u
 ...
 direzione="giu"  
       c1 = {}
       c2 = {} 
       for i=1,35 do c1[i]=0 c2[i]=0 end

    local function spostamento1()
      for i=1, 35 do

      	--if i==28 then print("c1= "..c1[i].." c2= "..c2[i].." direzione= "..direzione) end
    	ogg=obj[i] 

    	if ogg.mov=="muovere1" then 
    ...
    elseif direzione == "su" and  ogg.y~=nil then ogg.y=ogg.y-0.5 if ogg.scritta~=nil then ogg.scritta.y=ogg.scritta.y-0.5 end c2[i]=c2[i]+1 end
     ...
     local function spostamento2()
     for i=1, 35 do
    	ogg=obj[i] 
    	if ogg.mov=="muovere2"  then 
    	if c1[i]>=60 then c1[i]=0 direzione="su" end
    	if c2[i]>=60 then c2[i]=0 direzione="giu" end
         if direzione == "giu" and ogg.y~=nil then
        ogg.y=ogg.y+0.25 if ogg.scritta~=nil then ogg.scritta.y=ogg.scritta.y+0.25 end c1[i]=c1[i]+1
    elseif direzione == "su" and  ogg.y~=nil  then ogg.y=ogg.y-0.25 if ogg.scritta~=nil then ogg.scritta.y=ogg.scritta.y-0.25 end c2[i]=c2[i]+1 end
        end --ogg.name
      end --for
    end --function
     local function spostamento3()
      
      for i=1, 35 do
    	ogg=obj[i] 
    	if ogg.mov=="muovere3" then 
    	if c1[i]>=60 then c1[i]=0 direzione="su" end
    	if c2[i]>=60 then c2[i]=0 direzione="giu" end
         if direzione == "giu" then
        ogg.y=ogg.y+1 ogg.scritta.y=ogg.scritta.y+0.25 c1[i]=c1[i]+1
    elseif direzione == "su" then ogg.y=ogg.y-0.25 ogg.scritta.y=ogg.scritta.y+0.25 c2[i]=c2[i]+0.25 end
        end --ogg.name
      end --for
    end --function

Runtime:addEventListener("enterFrame",spostamento1)
Runtime:addEventListener("enterFrame",spostamento2)
--timer.performWithDelay( 10, function() Runtime:addEventListener("enterFrame",spostamento2) end )

	sfondo= myLevel:getLayerObject("Layer 1","spazio26_0").view
