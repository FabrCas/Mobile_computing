...
local channel1= audio.findFreeChannel(1)
  ...
  audio.play(levelSound,{channel= channel1, loops = -1})
	...
	myLevel:loadLevel("Level21") -- set your scene/level name here
	...
	obj[i].view.vita = obj[i].property['vita']
	if not (obj[i].view.name=="unbreak") then 
	print(obj[i].view.vita )

	if (obj[i].view.name=="speciale" or obj[i].view.vita==1 or mod_par=="arcade") then
	obj[i].view.life = 1
	else
	if (obj[i].view.vita=='5') then 
	obj[i].view.life = 5
	else 
    obj[i].view.life = 4
end 
...
elseif angolo <-45 and angolo >-135 then --u
  obj[i].view.scritta.rotation = obj[i].view.rotation +90
	...
	sfondo= myLevel:getLayerObject("Layer 1","spazio21_0").view
